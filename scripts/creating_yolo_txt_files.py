# -*- coding: utf-8 -*-
"""
Created on Wed Oct 19 12:08:06 2022
...
for berke in range(292):
    ...
    while iterator<len(data):
        ...
        img_width = neuron_crop.width
        img_height = neuron_crop.height
        
        x_center = round(((x_max+x_min)/2)/img_width,6)
        y_center = round(((y_max+y_min)/2)/img_height,6)
        width = round((x_max-x_min)/img_width,6)
        height = round((y_max-y_min)/img_height,6)

        print("0 "+str(x_center)+" "+str(y_center)+" "+str(width)+" "+str(height))
        with open("C:/OpenCV-Python/original_images/yolo_txt_files/"+img_files()[berke][:len(img_files()[berke])-4]+".txt","a") as f:
            f.writelines("0 "+str(x_center)+" "+str(y_center)+" "+str(width)+" "+str(height)+"\n")
        """
